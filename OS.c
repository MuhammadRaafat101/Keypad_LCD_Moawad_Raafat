...
void OS_Creat_Task(	uint16_t time_period, ptrToFunc_t task, uint8_t periorty)
{
	...
	if (first_empty < MAX_SIZE_OF_TASK)
	{
		if (first_empty == 0)
		{
...
void OS_Run(void)
{
	...
	while(1)
	{
		...
		{   /*check if the time of the functions comes */
			if( task_buffer[loop_index].time_period == task_buffer[loop_index].task_counter )
			{
				task_buffer[loop_index].task();
				/*if you call the func reset its counter to start from the begining*/
				task_buffer[loop_index].task_counter=0;
			}
		}
...
{
	uint8_t task_detected_flag = 0,loop_index=0;
